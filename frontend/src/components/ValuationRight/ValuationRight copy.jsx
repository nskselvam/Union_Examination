...
const ValuationRight = ({ questionMain, barcodeData, viewedCount = 0, totalCount = 0, currentPage, imgNumber, end_image, responseDataFromValuation, onFinalizationComplete, onMarksUpdate ,basicData }) => {
                                ...
                                {part.questions.map(question => (
                                    <div key={question.id}>
                                        <div style={{ 
                                            textAlign: 'center', 
                                            marginBottom: '0.3rem', 
                                            color: question.isCompulsory ? '#dc3545' : '#28a745', 
                                        ...
                                        <Form.Group style={{ marginBottom: 0 }}>
                                            <Form.Control
                                                ...
                                                style={{
                                                    ...
                                                    minHeight: '2.3rem',
                                                    borderRadius: '4px',
                                                    border: question.isCompulsory ? '2px solid #dc3545' : '1px solid #999',
                                                    backgroundColor: question.isCompulsory ? '#fff5f5' : '#ffffff',
                                                    fontSize: '0.85rem',
                                                    padding: '0.3rem'
                                                }}
                                            />
                        ...
                        <div>
                            ...
                            {calculationResult?.QuestionValidStatus && (() => {
                                ...
                                questionsToDisplay.forEach(item => {
                                    const key = `${item.qbno}:::${item.sub_section || ''}:::${item.add_sub_section || ''}`;
                                    calculationMap[key] = item;
                                });

                                // Use formData as the source of all questions to ensure subsections are shown
                                return formData.map((part, partIndex) => {
                                    // Calculate section-wise marks - handle both regular and 'ab' sections
                                    const sectionKey = `Section${part.section}`;
                                    const sectionKeyA = `${sectionKey}a`;
                                    const sectionKeyB = `${sectionKey}b`;
                                    
                                    let totalMarks = calculationResult.SectionWiseMarks?.[sectionKey] || 0;
                                    
                                    // If regular section has no marks, check if it's an 'ab' section
                                    if (totalMarks === 0) {
                                        const marksA = calculationResult.SectionWiseMarks?.[sectionKeyA] || 0;
                                        const marksB = calculationResult.SectionWiseMarks?.[sectionKeyB] || 0;
                                        if (marksA > 0 || marksB > 0) {
                                            totalMarks = marksA + marksB;
                                        }
                                    }
                                        ...
                                        <div key={partIndex} style={{
                                            ...
                                            <div style={{
                                                ...
                                                {part.questions.map((question, idx) => {
                                                    // Look up calculation data for this question
                                                    let calcItem = calculationMap[question.id];
                                                    
                                                    // If not found, try without the add_sub_section (for roman numerals i, ii, etc.)
                                                    if (!calcItem && question.id.includes(':::')) {
                                                        const parts = question.id.split(':::');
                                                        const qbno = parts[0];
                                                        const sub_section = parts[1] || '';
                                                        const alternateKey = `${qbno}:::${sub_section}:::`;
                                                        ...
                                                        <div key={idx} style={{ textAlign: 'center' }}>
                                                            ...
                                                            <input
                                                                ...
                                                                style={{
                                                                    ...
                                                                    textAlign: 'center',
                                                                    border: `3px solid ${isValid ? borderColor : '#6c757d'}`,
                                                                    borderRadius: '8px',
                                                                    backgroundColor: isValid ? backgroundColor : '#f8f9fa',
                                                                    fontWeight: '700',
                                                                    fontSize: '1.1rem',
                                                                    color: isValid ? textColor : '#6c757d',
                                                                    cursor: 'default',
