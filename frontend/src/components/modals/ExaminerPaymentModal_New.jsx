...
import { useGetEvaluatorDetailsSubjectCountQuery ,usePostExaminerPaymentChallanMutation,useExaminerPaymentUpdateMutation} from '../../redux-slice/userDashboardSlice'




const ExaminerPaymentModal = ({ show, onHide, paymentData }) => {

    const navigate = useNavigate();

    const [postExaminerPaymentChallan, { isLoading: isPosting }] = usePostExaminerPaymentChallanMutation();
    const [updateExaminerPayment, { isLoading: isUpdating }] = useExaminerPaymentUpdateMutation();
    ...
    const { data: subjectCountData, isLoading, isError } = useGetEvaluatorDetailsSubjectCountQuery(
        {

            "Eva_Id": paymentData?.evaluatorId || "",
            "frmdate": formatDate(paymentData?.fromDate),
            "todate": formatDate(paymentData?.toDate),
            "examiner_type": paymentData?.examiner_Type || "",
    ...
    useEffect(() => {
        ...
        setDaAmount(daPerDay * workingDays);
    ...
    const handlePostExaminerPaymentChallan = async (data) => {
        ...
        try {
            ...
            const result = await updateExaminerPayment(data).unwrap();
                        ...
                        <Table responsive>
                            <thead>
                                <tr>
                                    ...
                                    <th className="text-end">Rate/Paper</th>
                                    <th className="text-end">Amount (₹)</th>
                                </tr>
                            </thead>
                            <tbody>
                                {subjectCountData?.importfinal?.length > 0 ? (
                                    subjectCountData.importfinal.map((record, index) => (
                                        <tr key={index}>
                                            <td className="text-center">{index + 1}</td>
                                            <td>
                                                <Badge bg="primary" style={{ fontSize: '0.85rem', padding: '5px 10px' }}>
                                                    {record.subcode}
                                                </Badge>
                                            </td>
                                            <td>{record.sub_name}</td>
                                            <td className="text-center">
                                                <strong style={{ fontSize: '1rem' }}>{record.checked_scripts}</strong>
                                            </td>
                                            <td className="text-end">₹ {formatCurrency(record.rate_per_script).replace('₹', '').trim()}</td>
                                            <td className="text-end">
                                                <strong style={{ color: '#667eea', fontSize: '1.05rem' }}>
                                                    { record.eval_type === 'Y' ? `₹ ${formatCurrency(record.amount_calculated).replace('₹', '').trim()}` : 'Examiner Not Closed' }
                                                </strong>
                                            </td>
                                        </tr>
                                    ))
                                    ...
                                    <tr>
                                        <td colSpan="6" className="no-data-message">
                                            <FaInfoCircle size={40} style={{ marginBottom: '10px', opacity: 0.5 }} />
                                            <div>No valuation records found for the selected date range</div>
                                        </td>
                                    </tr>
                                )}
                            </tbody>
                    ...
                    <div className="total-card-modal">
                        ...
                        {daAmount && (
                            <div style={{ 
                                ...
                                >
                                    DA Amount: ({paymentData?.daAmount} X {subjectCountData?.importfinal?.[0]?.overall_working_days || 0}) =  ₹ {parseFloat(daAmount || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                                </Badge>
                                {taAmount    && (
                                    <Badge 
                                        bg="light" 
                    ...
                    {/* {subjectCountData.length > 0 && ( */}
                        <>
                            ...
                            <Button className="btn-download-custom" onClick={() => handlePostExaminerPaymentChallan({
                                ...
                                daTotal: daAmount,
                            })} disabled={isPosting || !subjectCountData?.paymentGenerated}>
                                {isPosting ? 'Generating PDF...' : ''}
...
export default ExaminerPaymentModal;
